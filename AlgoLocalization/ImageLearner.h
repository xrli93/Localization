...
namespace Localization
{
    ...
    class FREEImageLearner : public ImageLearner<Mat>
    {
    public:
        FREEImageLearner()
        {
            mDict.SetFeatureMethod(USE_FREE);
            mDict.SetRadius(RADIUS_FREE);
        }

        FREEImageLearner(float radius)
        {
            mDict.SetFeatureMethod(USE_FREE);
        ...
        Mat CalculateFeatures(const Mat& img)
        {
            ...
            if (USE_FREE)
            {
                ...
                //f2d = xfeatures2d::FREE::create(NUM_MAX_FREE, 4, 0.03, 10, 1.6);
            }
            else
            {
                f2d = xfeatures2d::SIFT::create(NUM_MAX_FREE, 4, 0.03, 10, 1.6);
            }
            ...
            //Ptr<Feature2D> f2d = ORB::create(NUM_MAX_FREE, 1.2, 8);
